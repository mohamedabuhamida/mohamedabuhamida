...
export default async function BlogPreviewPage({ params }: BlogPreviewPageProps) {
  ...
  const post = await getBlogBySlug(slug, { includeDrafts: true });
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#143b6a30,transparent_24%),linear-gradient(180deg,#02050a,#05101a_34%,#02050a)] px-6 pb-24 pt-10 sm:px-8">
      <article className="mx-auto max-w-6xl">
        <div className="mb-10 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <p className="text-[10px] uppercase tracking-[0.28em] text-accent/75">Dashboard Preview</p>
            <h1 className="text-2xl font-bold text-white">Previewing: {post.title}</h1>
            <p className="text-sm text-text/55">
              {post.published ? "This post is published publicly." : "This is a draft preview visible only inside the dashboard."}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {post.published ? (
              <Link
                href={`/blog/${post.slug}`}
                target="_blank"
                className="inline-flex items-center gap-2 rounded-2xl bg-accent px-4 py-3 text-sm font-semibold text-bg transition hover:bg-accent/85"
              >
        ...
        <header className="mx-auto max-w-3xl">
          <div className="rounded-[28px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-6 sm:p-8">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] uppercase tracking-[0.22em] text-text/45">
              <span className="text-accent/80">{post.published ? "Published Post" : "Draft Post"}</span>
              <span className="inline-flex items-center gap-2 normal-case tracking-normal text-[13px] text-text/45">
                ...
                {post.published ? "Published" : "Draft created"} {formatBlogDate(post.created_at)}
              </span>
              <span className="inline-flex items-center gap-2 normal-case tracking-normal text-[13px] text-text/45">
            ...
            <h2 className="mt-5 max-w-2xl text-4xl font-semibold tracking-[-0.05em] text-white sm:text-5xl">
              {post.title}
            </h2>
